...
#Applying a th root transformation to matrix
...
PCO_MaxN3 <- ggplot(PCO_MaxNFish) + 
  ...
  ylab(paste0("PCO2 (", 
              ...
              "% of total variation)")) +
  scale_x_continuous(breaks = seq(-0.25, 0.75, by = 0.25), limits = c(-0.37, 0.90)) +
  scale_y_continuous(breaks = seq(-0.25, 0.25, by = 0.25), limits = c(-0.37, 0.31)) +
...
PCO_MaxN4 <- ggplot(PCO_MaxNFish) + 
  ...
  geom_path(data = ordi$df_ellipse, aes(x = x, y = y, colour = Group), size = 1) +
  guides(color = guide_legend(order = 1, title = "MPA"),
         shape = guide_legend(order = 2, title = "Location")) +
  geom_segment(data = arrows_df, #Adding arrows
               x = Anchor[1], y = Anchor[2],
               mapping = aes(xend = X2, yend = Y2),
               arrow = arrow(length = unit(2, "mm")), #Adding arrow head
...
Plot3_4 <- ggarrange(PCO_MaxN3, NULL, PCO_MaxN4, widths = c(1, 0.1, 1), nrow = 1)

Plot3_4

#Saving PCO for biomass - method and management status
ggsave("Figures/PCO_AllFish.tiff",
       Plot3_4, device = "tiff", dpi = 300, width = 21, height = 10)
